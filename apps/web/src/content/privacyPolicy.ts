  ...
  sections: [
    ...
    },
    {
      ...
      bullets: [
        ...
        'Discord連携機能で扱う情報（サーバーID、チャンネルID、メンバーID、表示名等）※機能提供のために都度処理し、運営者サーバーに恒久的な利用者データとしては保存しません（障害対応・不正対策のためのログ記録を除く）。',
        '利用者が入力または生成する情報（ガチャ設定、景品情報、在庫情報、受け取り履歴、ユーザー表示名等）※通常は利用者端末内（ブラウザ）に保存され、運営者サーバーに恒久保存しません。共有・引き継ぎ機能を利用した場合は、利用者が操作した範囲で外部ストレージへ保存されます。',
    ...
    },
    {
      ...
      title: '6. Cookieの利用',
      paragraphs: [
        '本サービスでは、認証・セキュリティ・不正利用防止のためにCookieを利用します。',
        'Cookieを無効化している場合、本サービスの一部機能が利用できないことがあります。'
      ]
    },
    {
      id: 'analytics',
