...
export function ItemCard({ model, rarity, onEditImage, layout = 'vertical' }: ItemCardProps): JSX.Element {
  ...
  if (layout === 'horizontal') {
    return (
      <article data-item-id={model.itemId} data-riagu={model.isRiagu} className={clsx(baseArticleClass, 'w-full')}>
        {badgeStack}
        <div className="item-card__content flex items-center gap-4">
          <div
            className={clsx(
              'item-card__media flex aspect-square h-28 w-28 flex-shrink-0 items-center justify-center rounded-xl border border-border/60 bg-[#1b1b22] text-muted-foreground transition-colors',
          ...
          <div className="item-card__body flex flex-1 flex-col justify-between gap-3 py-1 text-left">
            ...
            <div className="item-card__actions flex flex-wrap gap-2 pt-1 text-xs">
              <button
                type="button"
  ...
  return (
    <article data-item-id={model.itemId} data-riagu={model.isRiagu} className={baseArticleClass}>
      {badgeStack}
