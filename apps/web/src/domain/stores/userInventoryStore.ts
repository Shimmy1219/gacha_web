...
export class UserInventoryStore extends PersistedStore<UserInventoriesStateV3 | undefined> {
  ...
  setInventoryItemCount(
    ...
    const { userId, inventoryId, rarityId, itemId, count, updatedAt, gachaId } = params;
    ...
    this.update((previous) => {
      ...
      const nextInventories = { ...baseState.inventories };
      const existingUserInventories = nextInventories[userId];
      const userInventories = existingUserInventories ? { ...existingUserInventories } : {};

      let snapshot = userInventories[inventoryId];
