...
import type { ReceiveMediaKind } from './types';
...
function ReceiveInventoryItemCard({ item }: { item: ReceiveInventoryItem }): JSX.Element {
...
export function ReceiveListPage(): JSX.Element {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const gachaMap = new Map<string, Map<string, ReceiveInventoryItem>>();
        ...
        for (const entry of historyEntries) {
          ...
          for (const item of items) {
            ...
            const itemMap = gachaMap.get(gachaName) ?? new Map<string, ReceiveInventoryItem>();
            ...
            if (!gachaMap.has(gachaName)) {
              gachaMap.set(gachaName, itemMap);
            }
          }
        }

        const nextGroups = Array.from(gachaMap.entries()).map(([gachaName, itemMap]) => {
          ...
          return { gachaName, items, totalCount };
        });
              ...
              >
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {group.items.map((item) => (
                    <ReceiveInventoryItemCard key={item.key} item={item} />
                  ))}
                </div>
              </div>
            ))}
