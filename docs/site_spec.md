...
- メインツールバーでは手動入力ボタン、全体エクスポート/インポート、Discord ログイン領域を提供し、`#controlsPage` がモバイルドロワーと連動する。【F:index.html†L175-L192】
...
### 3.3 入力とワークフロー
- カタログ貼り付け、手動入力貼り付け、リアルグッズ設定など一連のワークフローがモーダルで段階的に実装されている。【F:index.html†L328-L400】
...
- 手動入力解析では `splitLiveBlocks` と `parseLiveBlock` がユーザー名・連数・レアリティ別明細を抽出し、同一ユーザー・ガチャで加算できるように整形する。【F:src/parsers.js†L29-L57】
